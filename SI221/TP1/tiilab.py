...
#######################################################################################
#
def visusarbase(tabima,zparam,tagspe, *therest):
#
...
#        print("visusar sans second parametre'
    ...
    if kparam>0:
        ...
        malegende='Image seuillee : valmoy (%.3f) + %.3f sigma  (0.,%.2f)' %(valmoy,kparam,valseuil)
        if valseuil>valmax:
            BB[0,0]=valseuil
#            valseuil=valmax
#            malegende='Image sans seuillage'
        ...
        if itagseuil==2 :
            valseuil2=valmoy-kparam*valsig
            if valseuil2>valmin:
                masque=BB>valseuil2
                BB=BB*masque+(1-masque)*valseuil2
            else :
                valseuil2=valmin
            malegende='Image seuillee : valmoy (%.3f) +- %.3f sigma  (%.2f;%.2f)' %(valmoy,kparam,valseuil2,valseuil)
...
#        plt.imshow(BB)
        plt.imshow(BB,interpolation='nearest')        
        if ipart==2 :  
        ...
        if ipart>0 :
            ...
            noutlier=npy.sum(tabima>valseuil)
            fntot=float(ntot)
            fnoutlierpourcent=float(noutlier)*100./fntot
            baratin=u'%d pixels > %.1f '%(noutlier,valseuil)  
            seuilmaxhisto= seuilmax
            if  seuilmaxhisto > valmax :
                seuilmaxhisto = valmax
#            plt.hist(npy.reshape(tabima,ntot), bins=256, normed=True, color='wheat')
            ...
            plt.plot([valseuil,valseuil],[yyymin,yyymax],'k--')
...
###                plt.xlim([0.,seuilmaxhisto])
                baratin=u'%d pixels > %.1f (%.1f  %%) ,   %d pixels > %.1f (%.3f  %%)'%(noutlier,valseuil,fnoutlierpourcent,noutlierhisto,seuilmax,fnoutlierhistopourcent)
            else :
                baratin=u'%d pixels > %.1f (%.1f  %%)'%(noutlier,valseuil,fnoutlierpourcent)
...
def visusarhisto(tabima,zparam=-999):
    """
    affichage d'images plutot radar.  Si image complexe : affichage de la valeur absolue
    ...
    Arguments en entrée : 1 ou 2
        ...
        argument 2 (facultatif) : facteur de la formule <<valeur moyenne + fak * écart type >>
        Si ce facteur est nul, l'image ne sera pas seuillée
        Si ce facteur est négatif, l'image sera seuillée pour le max ET pour le min 
        
    Argument en sortie : le tableau affiché (avec seuillage)
        
    Utilisez visusarZ (même syntaxe) pour éviter le plt.show()     
    
    Utilisez visusarW (même syntaxe) pour n'avoir aucun affichage 
    """
    return visusarhistoZ(tabima,zparam,1) 
...
def visusarZ(tabima, *therest):
    ...
    if(len(therest)>1):
        itag=therest[1]
    return visusarbase(tabima,nnn,0,itag) 
       
def visusarZZ(tabima, *therest):
    nnn=-999
    if(len(therest)>0):
        nnn=therest[0]
    return visusarbase(tabima,nnn,0,2) 
...
def visusarhistoZ(tabima, *therest):
    nnn=3.
    nnnhisto=3.
    if(len(therest)>0):
        nnn=therest[0]
...
#    if(len(therest)>1):
#        itag=therest[1]
    return visusarbase(tabima,nnn,0,1,nnnhisto)     
####################################################################
    # fin outils visu
####################################################################
...
######################################################################################################## 
# MAI 2018 : utilitaires pour les labels et les dates 
#
#
import datetime
def lirelabeldate(nomimage): 
    ns=nomimage.rindex('.')
    nomspe=nomimage[0:ns]+'.label'
    try :
        f=open(nomspe);
    except IOError:
        legx=nomimage+': est un fichier non ouvrable'
        print(legx)
        return ''
    else:
        tablinebase=f.readlines()

    nligne=npy.size(tablinebase)
    tabline=npy.empty( nligne, dtype='U10')  
#
# attention : il peut y avoir deux types de labels ...
#  
#    print(tablinebase)
    for iut in range(nligne):
        idate=tablinebase[iut].index(' ')+1
        if idate>10 :
            idate=0
        tabline[iut]=tablinebase[iut][idate:idate+10]
    return tabline
    
    
def deltajours(tabline): 
    nligne=npy.size(tabline)
    tabval=npy.zeros(nligne)
    
    rdatebase = datetime.datetime.strptime(tabline[0], "%Y-%m-%d").date()
    for iut in range(1,nligne):
        rdate = datetime.datetime.strptime(tabline[iut], "%Y-%m-%d").date()
        delta =  (rdate - rdatebase).days
        tabval[iut]=delta
...
######################################################################################################## 
#
# JUIN 2018 : un programme simple pour balayer rapidement une pile temporelle
#    
def visupile(imatot)  :

    nsz=npy.shape(imatot)
    ndims=npy.size(nsz)
    if ndims<3 :
        nkan0=1
    else : 
        nkan0=npy.size(imatot,2)
    ...
    while istop==0 :
        visusarZ(imatot[:,:,kut])
        plt.title(u'Image %d/%d \n Souris pour continuer, clavier pour arreter'%(kut+1, nkan0))
    #    plt.draw()
